    ...
    if (typeof thisClient === 'boolean' && thisClient === false) {
      ...
      if (validation) {
        if (paramScheduledTime > now.getTime() + 60 * 1 * 1000) {
          await addSchedule(scheduleID, paramClientID, paramStopID, paramLocationName, paramRouteID, paramRouteName, paramDirection, paramEstimateTime, paramPhoto, paramScheduledTime, env);
          responseObject = {
