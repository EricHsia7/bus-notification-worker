import { discardExpiredSchedules, discardExpiredToken, listSchedules } from './database';
...
export async function send(event, env, ctx) {
  ...
  for (const schedule of schedules) {
    ...
    url.searchParams.set('permalink', `0@${schedule.RouteID.toString(16)}~${schedule.RouteName}`);
  ...
  await discardExpiredToken(now, env);
}
